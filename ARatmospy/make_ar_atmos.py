...
def make_ar_atmos(
    ...
    if telescope == "GPI":
        ...
        bigDs = 0.990
        nacross = (
            16.0  ## this will have to change when 32x (30 across) mode is operational
        )
    ...
    for i in np.arange(n_layers, dtype=int):
        ...
        if savelayers:
            print(
                "Writing layer {} file to {}.fits".format(str(i), layeroutfile + str(i))
            )
            phase[:, :, i, :].shape
    ...
    if dopsd:
        ...
        kx, ky = generate_grids(
            bign, scalefac=2 * np.pi / (bign * pscale), freqshift=True
        )
        ...
        eff_r0 = (r0s ** (-5.0 / 3.0)).sum() ** (-3.0 / 5.0)
        mp.clf()
        mp.yscale("log")
        mp.xscale("log")
        mp.plot(kr, varpsd, "b.")
        mp.plot(
            kr,
            0.490 * (eff_r0) ** (-5.0 / 3.0) * kr ** (-11.0 / 3.0) * scale_for_nm,
            "r-",
        )
        mp.ylim(1e-8, 1e2)
        mp.xlim(1, 200)
    ...
    print(
        "Done generating {}-layer AR atmopshere file with params n={}, m={}, rate={}, time={}s".format(
