...
if FTmmode == "w+":
    ...
    for t in np.arange(timesteps):
        ...
        if fds:
            phapFT[t, :, :] = (
                np.fft.fft2(wf * aperture) * freq_dom_scaling / (phx * phy)
            )
        else:
    ...
    for t in np.arange(timesteps):
        ...
        if fds:
            phdtFT[t, :, :] = (
                np.fft.fft2(dt.detilt(dp.depiston(wf * aperture, aperture), aperture))
                * freq_dom_scaling
                / (phx * phy)
            )
        else:
            phdtFT[t, :, :] = np.fft.fft2(
                dt.detilt(dp.depiston(wf * aperture, aperture), aperture)
            ) / (phx * phy)
...
pt = mp.plot(kr, 0.490 * (eff_r0) ** (-5.0 / 3.0) * kr ** (-11.0 / 3.0), "k-")

mp.legend(
    [po[0], pa[0], pd[0], pt[0]],
    ["Open loop (OL)", "OL + aperture", "OL + aperture/depiston/detilt", "Theoretical"],
    loc="upper right",
)
...
while mode is not "q":
    ...
    mp.plot(
        hz,
        this_psd[:, k, l]
        / np.abs(1 - alpha_mag * np.exp(-1j * 2 * np.pi * hz / rate)) ** 2,
        "r-",
    )
    mp.plot(
        hz,
