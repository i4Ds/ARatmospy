...
psd8k = [
    ar8k + "ar-atmos1l-0.99-48-20141110_172347",
    ar8k + "ar-atmos1l-0.999-48-20141110_173341",
    ar8k
    + "gpi_build_mag8_rate2048_exptime4.00000_atm0_plain_aplcgrey_hband+input_phase",  # 3-layer
    ar8k + "gpi_build_mag8_rate2048_exptime4.00000_atm0_plain_hband+input_phase",
]  # 1-layer
...
while mode is not "q":
    ...
    mp.ylabel("Power")
    mp.title(
        "Temporal PSD (rate=2048, perlen=1024) for mode k=%s, l=%s" % (mode[0], mode[1])
    )
    ...
    mp.plot(
        ...
        * kr[k, l] ** (-11.0 / 3.0)
        * 1e-5
        / np.abs(1 - 0.99 * np.exp(-1j * omega)) ** 2,
        "r--",
    )
    ...
    mp.plot(
        ...
        * kr[k, l] ** (-11.0 / 3.0)
        * 1e-5
        / np.abs(1 - 0.9991 * np.exp(-1j * omega)) ** 2,
        "b--",
    )
...
psd21cl = [
    f21s
    + "gpi_ar_mag8_rate1000_exptime22.0000_amag0.99_atm0_plain_aplcgrey_hband+measurements",
    f21s
...
psd21clo = [
    f21s
    + "gpi_ar_mag8_rate1000_exptime22.0000_amag0.99_atm0_ttopt_optfil+ofc_ar_mag8_rate1000_amag0.99_aplcgrey_hband+measurements",
    f21s
...
psd21clk = [
    f21s
    + "gpi_ar_mag8_rate1000_exptime22.0000_amag0.99_atm0_ttopt_kalman+pfc_ar_mag8_rate1000_amag0.99_aplcgrey_hband+measurements",
    f21s
...
psdffcl = [
    "gpi_build_mag8_rate1500_exptime4.00000_atm0_plain_aplcgrey_hband+measurements"
]

# telemetry psds phase in microns
fgpi = rootdir + "Telemetry/gpi/"
psdgpi = [
    fgpi + "ugp_When_2013.11.12_0.27.6_phase",
...
while mode is not "q":
    ...
    mp.ylabel("Power")
    mp.title(
        "3-layer Closed Loop Temporal PSD for mode k=%s, l=%s" % (mode[0], mode[1])
    )
    (p99,) = mp.plot(shz, psd99[ahz, k, l], "r-")
    (p99cp,) = mp.plot(hzp, np.cumsum(psd99[0 : perlen / 2, k, l]), "r--")
    psd99cn = psd99[perlen / 2 : perlen, k, l]
