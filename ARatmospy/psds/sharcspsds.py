...
#    print(fname)
#    if np.isnan(np.sum(hdulist[0].data)):
#        print('Damn, a Nan')
...
for fname in fitsfiles:
    ...
    if os.path.isfile(PSDfile):
        ...
        timesteps = phdim[0]
        print("Computing FT")
        phFT = np.memmap(FTfile, dtype=np.complex128, mode="w+", shape=(timesteps, phx, phy))
        for t in np.arange(timesteps):
            wf = hdulist[0].data[t, :, :]
            phFT[t, :, :] = np.fft.fft2(wf) * freq_dom_scaling / (phx * phy)
        print("Doing PSD")
        dtpsd = np.memmap(PSDfile, dtype=np.float64, mode="w+", shape=(perlen, phx, phy))
        for k in np.arange(phx):
            ...
            for l in np.arange(phy):
                dtpsd[:, k, l] = gapu.gen_avg_per_unb(
