...
def create_compressed_cp_for_cdr(gltype, fatype):
    ...
    paracube[:, 0] = np.array(
        [
            (r0s[layer1] ** (-5.0 / 3.0)).sum() ** (-3.0 / 5.0),
            (r0s[layer2] ** (-5.0 / 3.0)).sum() ** (-3.0 / 5.0),
            (r0s[layer3] ** (-5.0 / 3.0)).sum() ** (-3.0 / 5.0),
        ]
    )
    paracube[:, 1] = np.array([vels[layer1dom], vels[layer2dom], vels[layer3dom]])
    paracube[:, 2] = np.array([angles[layer1dom], angles[layer2dom], angles[layer3dom]])
...
def cdr_create_parameters(atmtype):
    if atmtype == 0:
        ...
        cp_params = create_compressed_cp_for_cdr(0, 0)
        cp_params[:, 0] = cp_params[:, 0] * atmtype ** (-3.0 / 5.0)
